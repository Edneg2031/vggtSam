...
当前 hard-memory baseline 采用更保守的逐实例回退：每个实例维护独立的
SAM3 `obj_id` 与 memory；只有该实例的 SAM3 original mask 完全为空、几何候选
通过投影支持度检查，且恢复 mask 覆盖至少一半几何支持点时，才写回该实例
memory。最终输出在 original mask 非空时始终保留 original；只有该实例缺失且
当前帧几何候选也通过门控时，才使用 hard-memory mask，避免在目标真实离开画面
时产生假阳性。多实例联合 ICP 的门控只决定共享相机修正，不会混合不同实例的
memory。
