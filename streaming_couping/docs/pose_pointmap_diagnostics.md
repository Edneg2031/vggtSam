...
这是 3×3 加权线性法方程。可部署主分支使用 predicted `K/R`、归一化 point
confidence，并反复保留射线残差最低的 80%。若有效点少于 1024，或法方程条件数
超过 `1e8`，该帧明确回退 raw camera center。最后保持 world-to-camera rotation
不变，仅更新：
...
| `ray_predicted_k_all` | predicted | predicted | 否 | trimming 消融 |
| `ray_predicted_k_trimmed` | predicted | predicted | 是 | 可部署主方法 |
...
8. `ray_predicted_k_trimmed` 的固定 point-Sim3 ATE/RPE 优于 raw：
   - point head 可以作为 camera-head translation 的无训练后处理约束。
9. trimmed 优于 all，且 shuffled 明显恶化：
   - 改善来自正确像素—点射线对应和稳健筛点，而非任意中心重拟合。
