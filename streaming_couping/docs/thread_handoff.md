...
```text
scripts/run_instance_pose_refinement.py
...
| mode | ATE | adjacent RPE | all-pairs mean | median |
|---|---:|---:|---:|---:|
| `ray_only` | 0.175915 | 0.080938 | 11.345° | 7.383° |
| final V3 | **0.173651** | 0.080995 | **10.780°** | **6.942°** |

V3 的 ATE 改善 1.29%，all-pairs mean 改善 4.99%，median 改善 5.98%，RPE
基本持平。状态机验证通过：130 wardrobe carry #1、140 carry #2、210 reset、240
拒绝无历史冲突。`v3_temporal_validated_a050` 已冻结为当前主方法。
...
V1/V2、consensus-only、unvalidated-map、shuffled、GT oracle 和 alpha sweep 已从
运行时代码删除，历史结论保留在文档/Git。
...
run_instance_pose_refinement_ablation.py
```
...
命令复用 tracking cache，只计算 ray baseline 与最终 V3，并继续导出实例 PLY。

## 新分支：persistent instance token -> CameraHead

旧的 VGGT dense token -> SAM3 FPN2 fusion未通过严格因果标准：aligned IoU
`0.9262`、shuffled `0.9238`，两者只差 `0.0024`。因此新分支不再向 SAM3 注入
任何 token；SAM3 仅提供当前已验证的 recovered mask、persistent ID、score和
frozen pooled appearance。
...
configs/instance_token_pose.yaml
commands_instance_token_pose.txt
docs/instance_token_pose.md
```

首版在 final aggregator hidden camera token与 frozen CameraHead之间进行
zero-initialized cross-attention。persistent token显式编码 current/history/delta
geometry；reference 帧只初始化 memory，不修改 pose。`camera_geometry_only`、
`camera_sam_only`、`camera_token_fusion`和`all_token_fusion`分别训练；zero、
module-off、shuffled-ID、shuffled-time均使用同一个 aligned checkpoint做推理扰动。

当前 checked-in 7 帧 clip只允许判断可学习性，正式泛化结论需要在 YAML 中增加不同
场景的 train/val/test clips。服务器一键入口：
