...
StreamVGGT pointmap X_i + pixel ray d_i(predicted K/R)
  ...
  -> robust 80% residual trimming
  -> keep R, replace t with -R C
```

可部署主分支为 `ray_predicted_k_trimmed`，不使用 GT。一次运行同时输出 raw、
untrimmed、GT-K、GT-R、GT-K+R 和 spatially-shuffled pointmap 分支。GT 分支
只诊断 intrinsics/rotation 上限，shuffled 是破坏像素—点对应的负对照。若点数
少于 1024 或条件数超过 `1e8`，明确回退 raw center。
