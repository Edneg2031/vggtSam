...
**+14.25% 翻到 −2.38%**。原因是分割器对 prompt 的响应**不可加**，而且现在**两个
机制都实测到了**（`sam3_candidate_ledger`，v5 = v1 + `cabinet`，150 帧）：
...
- **判重**：`wardrobe` 被判重丢掉（输给一条出生更早的 track）。
- **名额**：`cabinet` 一个词回了 **15 条 track**，accepted 合计 **1+0+3+1+5+6 = 16
  —— 正好撞满全局上限**，`rug` 也被挤掉一条。

结果 d_ATE **−11.87%**，五变体全 NO_GO。**但这一轮分不清归因**：丢 `wardrobe` 和加
`cabinet` 同时发生，要分开得让 cabinet 进来而 wardrobe 不丢（抬上限或改判重排序）。

**加词的天花板还不是这两个机制，是分割器本身。** v4 加了清单里 kept 帧数最高的
`table`（GT 侧 4477 px 级别的物体），**SAM 一条 track 都没回** —— 文本 grounding 在
阈值 0.5 上没有给出任何检测。而同一轮 oracle 用 **GT mask** 给 `table` 出了约
**99 条提案**。同一个词、同一个场景、同一份几何：**GT 找得到，SAM 找不到。**
...
| 加 prompt 能改善 | v4（加 `table`）、v5（加 `cabinet`） | **否**。v4 的词一条 track 都没产出（文本 grounding 没检出）；v5 反而把已有类别挤掉、结果 −11.87% |
