...
| 分支 | 旋钮 | 含义 |
|---|---|---|
| `baseline` | — | 联合解 6DoF；参考集固定为前 5 帧 anchor + 近期 history |
| `fresh` | `max_reference_age_frames=15`<br>`anchor_refresh_interval_frames=20` | 参考集**只看最近 15 帧**，anchor 每 20 帧重取 |
| `factorized` | `proposal_mode=rotation_then_translation` | **先解旋转再解平移**，而不是联合解 |
| `translation_only` | `proposal_mode=translation_only` | **强制 ω=0**，只解平移 |
| `fresh_factor` | `fresh` + `factorized` | 两者一起 |
| `oracle_mask` | 用 **GT mask** 替换 SAM | **不是方法分支**，是诊断上界 |

**列的读法**：`prop_err` = **单个提案**对 GT 的中位平移误差 —— 这是提案侧的直接读数；
...
| 分支 | prop_err | cons_err | accepted | d_ATE | d_sim3 | 判定 | **这个分支说明了什么** |
|---|---:|---:|---:|---:|---:|---|---|
