...
| 分支 | 它要检验的假设 | 怎么检验 |
|---|---|---|
| `baseline` | —（**不是消融**，是方法本身） | 联合解 6DoF；参考集 = 前 5 帧 anchor（权重 1.0）+ 近期 history（0.5） |
| `fresh` | **"参考集太旧"** —— 归因发现 anchor 年龄与提案误差类内 ρ = **0.79**，是最强的预测因子，所以怀疑提案是在对着过时的几何拟合 | 参考集**只看最近 15 帧**，anchor 每 20 帧重取（年龄 37.5 → 8） |
| `factorized` | **"旋转和平移在联合解里互相干扰"** —— 旋转本来就是更差的一半（共识旋转误差 0.611° vs raw RPE 0.318°） | **先解旋转、再解平移**，分两步而不是联合 |
| `translation_only` | **"旋转估得太差，不如不要"** —— 既然旋转比噪声底还差，干脆不修它 | **强制 ω=0**，只解平移 |
...
| `fresh` | 0.0891 | 0.0811 | 34 | +3.46% | −1.96% | NO_GO | **更差，且否证了"参考太旧"这个假设**。anchor 年龄 37.5→8、相关性 0.79→0.17，但 `prop_err` **几乎没动**（0.0893→0.0891）。参考年龄不是瓶颈 |
