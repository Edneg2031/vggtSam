...
这是最自然的下一步想法，所以值得说清楚为什么**不做**——而且这个"不做"是量出来的，
不是推出来的。

**先说"闭环"在这里指什么。** 一个环路意味着：第 t 帧做出的修正，会改变第 t+1 帧
**怎么估计**。现在不是这样——**所有提案都在任何修正存在之前就一次性算完了**，
所以修正再准，也影响不到后面帧的提案。

（要分清楚：修正**在传播上确实是因果的**，累积器会把它带到后面每一帧；开环的是
**估计**那一半。§2.0 里那句"传播上闭环、估计上开环"说的就是这个。）

**为什么想做。** 单个提案的中位误差是 0.089 m，而要修的 raw ATE 只有 0.109 m ——
提案质量就是瓶颈。那么"让修正把产生提案的东西改好一点，提案是不是就准了？"
是顺理成章的。
...
|---|---|---|
| SAM 的 mask | 只有 RGB | **不** |
| depth | 图像 + 模型内部状态（`forward_chunk` **不含位姿**） | **不** |
| **参考云** | **过去帧的位姿** | **会** ← 唯一 |

所以"闭环"在这条 pipeline 上只剩一件事：**用修正后的轨迹重新摆放参考云，再解一遍
提案。** 观测点、mask、depth 三者逐位不变，所以这件事**纯 CPU 就能做**。

**做了什么。** 第一轮的修正轨迹（`robust_semantic`，d_ATE 0.09265）当作新基座，
用**同一批相机系点**重解提案。然后走**和第一轮完全相同的 Stage 2b 路径** —— 只差
基座和提案。
...
那个空对照是必要的：重解路径是从诊断**重建**观测和配置的，不是复用 stage 2a 的，
所以它自己有偏差。实测**只有 +0.1%**，所以 −18% 可以归给"把修正喂回去"这个动作。
（顺带，空对照的 `base_shift` 是 2e-6 m，说明两种 c2w 表示的换算是对的。）

**结果：残差降 18%，轨迹反而差 3.6%，五个共识变体全部 NO_GO。**

**这意味着什么。** 环路**确实**携带了第一轮没用到的信息 —— 这不是噪声。但这个信息
**没有变成更好的位姿，而是把它推坏了**。又回到了整份分析反复出现的那个模式：
**你把某个量优化下去，不等于你关心的指标会变好。**

至于它是怎么变坏的（是门控选出的那批修正更差，还是这些修正叠加起来互相打架），
**这一轮的数据分不出来**，所以不猜。

**加上原理上的上限**（上表：只有参考云这一半能动），结论是：**真流式不值得做。**
