#!/usr/bin/env python3
"""Small CPU smoke test for the V2 causal recovery policy."""
...
def main() -> None:
    ...
    assert stats["recovery_success_count"] == 2, stats
    assert stats["normal_frame_unchanged_ratio"] == 1.0, stats
    assert torch.equal(result["masks_output"][0, 0], output_masks[0, 0])
    assert torch.equal(result["masks_output"][2, 0], output_masks[2, 0])
    assert bool(result["masks_output"][1, 0].any())
    assert bool(result["masks_output"][3, 0].any())
    assert result["objects"][0]["observation_count"] == 4
    print(
        "V2 geometry-recovery smoke passed "
        f"triggers={stats['recovery_trigger_count']} "
        f"applied={stats['recovery_success_count']} "
