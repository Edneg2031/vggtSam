...
def prepare_scannetpp_2d(config: ScanNetPP2DConfig) -> Dict[str, Any]:
    ...
    manifest_path = config.output_root / (
        "preflight_manifest.json" if config.dry_run else "manifest.json"
    )
    existing_scenes: Dict[str, Dict[str, Any]] = {}
    if not config.dry_run and manifest_path.is_file():
    ...
    for scene_id in tqdm(scene_ids, desc="scene"):
        ...
        existing_scenes[str(scene_id)] = scene_manifest

    manifest["scenes"] = [existing_scenes[key] for key in sorted(existing_scenes)]
...
def _resolve_scene_ids(config: ScanNetPP2DConfig) -> List[str]:
    ...
    elif config.scene_list and Path(config.scene_list).is_file():
        scene_ids = [
...
def _build_config(args: argparse.Namespace) -> ScanNetPP2DConfig:
    ...
    payload["data_root"] = _expand_path(payload["data_root"])
    payload["output_root"] = _expand_path(payload["output_root"])
    if payload.get("annotation_root"):
        payload["annotation_root"] = _expand_path(payload["annotation_root"])
    if payload.get("scene_list"):
        payload["scene_list"] = _expand_path(payload["scene_list"])
    if payload.get("metadata_root"):
        payload["metadata_root"] = _expand_path(payload["metadata_root"])
    if payload.get("semantic_classes_file"):
        payload["semantic_classes_file"] = _expand_path(
            payload["semantic_classes_file"]
        )
